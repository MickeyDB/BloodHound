...
const getSelectorForObjectId = (
    selectors: AssetGroupTagSelector[] | undefined,
    objectId: string
): AssetGroupTagSelector | undefined => {
    return selectors?.find((selector) =>
        selector.seeds?.some((seed) => seed.type === SeedTypeObjectId && seed.value === objectId)
    );
};
        ...
        () =>
            apiClient
                .listAssetGroupMembers(decoyAssetGroup!.id, undefined, {
                    params: {
                        object_id: `object_id=eq:${objectId}`,
                    },
                })
    ...
    const agtSelectorsQuery = useQuery(
        ...
        () =>
            apiClient
                ...
                .then((res) => getSelectorForObjectId(res.data.data.selectors, objectId)),
        {
            enabled:
    ...
    const toggleMutation = useMutation<unknown, unknown, boolean, { previousDecoyState: boolean | undefined }>(
        ...
        },
        {
            ...
            onSuccess: (_data, checked) => {
                ...
                onDecoyUpdated();
                addNotification(
                    checked ? 'User marked as decoy.' : 'User unmarked as decoy.',
        ...
        <Box
            ...
            <Box alignItems='center' display='flex' gap={1} minWidth={0}>
                ...
                <Typography
                    ...
                    variant='caption'
                    sx={{
                        fontWeight: isMarkedDecoy ? 600 : 500,
                        lineHeight: 1.3,
                    }}>
                    {isMarkedDecoy
                        ? 'This user is marked as a decoy.'
                    ...
                    <FormControlLabel
                        control={
                            <Switch
                                ...
                                inputProps={{ 'aria-label': 'Mark user as decoy' }}
